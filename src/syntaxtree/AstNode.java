...
public abstract class AstNode implements TreeDisplayable {
  //protected static ErrorMsg errorMsg;
  //public static void setErrorObj(ErrorMsg eo) {errorMsg = eo;}
...
/*
 * 
    Absyn[] theLinks = this.links();
    if (theLinks != null) {
      for (int i = 0; i < theLinks.length; i++) {
	Absyn obj = theLinks[i];
	if (obj == null) {
	  linkString += ";(null)";
	}
	else if (obj instanceof NamedDec) {
	  linkString += ";#"+((NamedDec)obj).id;
	}
	else if (obj instanceof LoopingExp) {
	  linkString += ";#L"+((LoopingExp)obj).id;
	}
	else {
	  linkString += ";?";
	}
